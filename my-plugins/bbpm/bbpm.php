...
INDEX ( `pm_to` , `pm_from`, `reply_to` )
			...
			case '0.1-alpha5':
				...
				foreach ( $messages as $message ) {
					...
					bb_update_usermeta( $message->pm_to, $bbdb->prefix . 'bbpm_last_read_' . $message->pm_thread, $last_read );
					bb_update_usermeta( $message->pm_from, $bbdb->prefix . 'bbpm_last_read_' . $message->pm_thread, $last_read );
				}
	...
	function send_message( $id_reciever, $title, $message ) {
		...
		bb_update_usermeta( bb_get_current_user_info( 'ID' ), $bbdb->prefix . 'bbpm_last_read_' . $pm['pm_thread'], $msg->ID );
	...
	function get_last_read( $thread_ID ) {
		global $bbdb;
		return (int)bb_get_usermeta( bb_get_current_user_info( 'ID' ), $bbdb->prefix . 'bbpm_last_read_' . (int)$thread_ID );
	}
	...
	function mark_read( $thread_ID ) {
		...
		if ( $this->get_last_read( $thread_ID ) != $this->get_thread_meta( $thread_ID, 'last_message' ) ) {
			/* printf( 'Marking as read. threadID=%1$s, userID=%2$s, metadataKey=%3$s, whatever=%4$s', $thread_ID, (bb_get_current_user_info('ID')), ($bbdb->prefix . 'bbpm_last_read_' . (int)$thread_ID), ((int)$this->get_thread_meta( $thread_ID, 'last_message'))); */
			bb_update_usermeta( bb_get_current_user_info( 'ID' ), $bbdb->prefix . 'bbpm_last_read_' . (int)$thread_ID, (int)$this->get_thread_meta( $thread_ID, 'last_message' ) );
		} else {
			/* printf( 'Not marking as read: %1$s', $thread_ID ); */
		}
	}
